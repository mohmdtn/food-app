...
export const OrderTakeAway = () => {
  ...
  return (
    <section className={`flex-cstm ${order_page.page === "takeAway" ? "d-flex" : "d-none"}`}>
      {/* change sub page buttons */}
      <section className="buttons px-4 pb-3">
        <button className={`${order_page.sub === "all" && "active"}`} onClick={() => showOrderPage("takeAway", "all")}>سبد خرید</button>
        <button className={`${order_page.sub === "me" && "active"}`} onClick={() => showOrderPage("takeAway", "me")}>سهم من</button>
      </section>
      {/* change sub page buttons */}
